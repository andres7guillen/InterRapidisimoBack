...
WORKDIR /src
COPY ["LambdaCalculator.csproj", "LambdaCalculator/"]
RUN dotnet restore "LambdaCalculator/LambdaCalculator.csproj"

WORKDIR "/src/LambdaCalculator"
COPY . .
RUN dotnet build "LambdaCalculator.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "LambdaCalculator.csproj" \
...
Execute unit tests
```
cd "LambdaCalculator/test/LambdaCalculator.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "LambdaCalculator/src/LambdaCalculator"

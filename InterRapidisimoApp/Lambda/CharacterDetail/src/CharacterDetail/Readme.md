...
WORKDIR /src
COPY ["CharacterDetail.csproj", "CharacterDetail/"]
RUN dotnet restore "CharacterDetail/CharacterDetail.csproj"

WORKDIR "/src/CharacterDetail"
COPY . .
RUN dotnet build "CharacterDetail.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "CharacterDetail.csproj" \
...
Execute unit tests
```
cd "CharacterDetail/test/CharacterDetail.Tests"
dotnet test
```

Deploy function to AWS Lambda
```
cd "CharacterDetail/src/CharacterDetail"
